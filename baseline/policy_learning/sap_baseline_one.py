...
path = ["../../GloVe/glove.6B.200d.txt" , "./SAPDATA/train/seq.in" , "./SAPDATA/train/seq.out" , "./SAPDATA/train/intent","./SAPDATA/train/info"]
t_path = ["../../GloVe/glove.6B.200d.txt" , "./SAPDATA/test/seq.in" , "./SAPDATA/test/seq.out" , "./SAPDATA/test/intent","./SAPDATA/test/info"]
...
x_in = tf.placeholder('float', [None, 27])
sap_y = tf.placeholder('float',[None, 27])

#define weights
weights = {
    'sap_weight': tf.Variable(tf.random_normal([27, 27]), name='sap_weight')
}
biases = {
    'sap_biase' : tf.Variable(tf.random_normal([27]),name='sap_biase')
}

sap_pred = tf.matmul(x_in,weights['sap_weight']) + biases['sap_biase']
...
with tf.Session(config=config) as sess:
    ...
    while iterations < epoc:
        ...
        for step,(batch_x,batch_info) in enumerate(Data.get_batch(batch_size)):
            ...
            _,c,p = sess.run([sap_optimizer,sap_cost,_sap_pred],feed_dict={x_in:sap_x,sap_y:system_action})
            ...
            if step % 50 == 0 and step != 0:
                ...
                label_list = []
                # f = open('./sapout.txt','w')
                for test_step,(test_batch,test_batch_info) in enumerate(t_data.get_batch(256)):
                    test_batch = np.transpose(test_batch,(1,0,2))
                    test_x = test_batch[2]
                    test_system_action = test_batch[-1]
                    system_action_prediction = sess.run(_sap_pred,feed_dict={x_in:test_x,sap_y:test_system_action})
