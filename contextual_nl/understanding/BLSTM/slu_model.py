...
class slu_model(object):
    ...
    def hist_cnn(self, idx):
        with tf.variable_scope('single_model'):
            if idx != 0:
                tf.get_variable_scope().reuse_variables()
            history_nl = tf.unstack(self.hist_nl, axis=1)[idx]
            inputs = tf.nn.embedding_lookup(self.embedding_matrix, history_nl)
            pooled_outputs = list()
    ...
    def role_model(self):
        self.unstack_cnn_hist = list()
        for i in range(self.hist_len * 2):
            self.unstack_cnn_hist.append(self.hist_cnn(i))
        
        stack_cnn_hist = tf.sigmoid(tf.stack(self.unstack_cnn_hist, axis=1))
        
        with tf.variable_scope("history_rnn_nl"):
            lstm_fw_cell = rnn.BasicLSTMCell(self.hidden_size)
            lstm_bw_cell = rnn.BasicLSTMCell(self.hidden_size)
            
            _, final_states = tf.nn.bidirectional_dynamic_rnn(lstm_fw_cell, lstm_bw_cell, stack_cnn_hist, dtype=tf.float32)
            final_fw = tf.concat(final_states[0], axis=1)
            final_bw = tf.concat(final_states[1], axis=1)
            outputs = tf.concat([final_fw, final_bw], axis=1) # concatenate forward and backward final states
