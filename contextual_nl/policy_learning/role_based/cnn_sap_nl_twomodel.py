...
nl_tourist = nl_cnn(tourist_in,'Tourist')
nl_guide = nl_cnn(guide_in,'Guide')
...
sap_tourist = sap_birnn(tourist_sap_in,"tourist_sap")
sap_guide = sap_birnn(guide_sap_in,"guide_sap")

sap_pred = tf.matmul(tf.concat([sap_tourist,sap_guide],-1),weights['sap_weight']) + biases['sap_biase']
...
with tf.Session(config=config) as sess:
    ...
    while iterations < epoc:
        ...
        for step,(batch_i,batch_nl) in enumerate(Data.get_batch(batch_size)):
            ...
            nl_tourist_in = batch_nl[0:3]
            nl_guide_in = batch_nl[3:6]
            tourist_out = []
            guide_out = []
            for i in range(len(nl_tourist_in)):
                t_o,g_o = sess.run([nl_tourist,nl_guide],feed_dict={tourist_in:nl_tourist_in[i],guide_in:nl_guide_in[i]})
                tourist_out.append(t_o)
                guide_out.append(g_o)
            #3xbatchx2outlayer

            tourist_x = np.transpose(tourist_out,(1,0,2))
            guide_x = np.transpose(guide_out,(1,0,2))
            batch_i = np.transpose(batch_i,(1,0,2))
            system_action = batch_i[-1]
            _,c,p = sess.run([sap_optimizer,sap_cost,_sap_pred],feed_dict={tourist_sap_in:tourist_x,guide_sap_in:guide_x,sap_y:system_action})
            # print (c)
            ...
            if step % 50 == 0 and step != 0:
                ...
                for test_step,(test_batch_i,test_batch_nl) in enumerate(t_data.get_batch(batch_size)):
                    ...
                    test_nl_tourist = test_batch_nl[0:3]
                    test_nl_guide = test_batch_nl[3:6]
                    test_tourist_out = []
                    test_guide_out = []
                    for i in range(len(test_nl_tourist)):
                        t_o,g_o = sess.run([nl_tourist,nl_guide],feed_dict={tourist_in:test_nl_tourist[i],guide_in:test_nl_guide[i]})
                        test_tourist_out.append(t_o)
                        test_guide_out.append(g_o)
                    test_tourist_x = np.transpose(test_tourist_out,(1,0,2))
                    test_guide_x = np.transpose(test_guide_out,(1,0,2))
                    ...
                    system_action_prediction = sess.run(_sap_pred,feed_dict={tourist_sap_in:test_tourist_x,guide_sap_in:test_guide_x,sap_y:test_system_action})
