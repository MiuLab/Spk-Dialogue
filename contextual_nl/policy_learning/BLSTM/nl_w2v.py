...
#allx7x40x200
  def get_batch(self,batch_size,shuffle=True):
    if shuffle == True:
      ...
      random.shuffle(self.encoded,lambda : r)
    data_size = len(self.intentvalue)
    num_batches_per_epoch = int((data_size-1)/batch_size)
    for batch_index in range(num_batches_per_epoch):
      start_index = batch_index*batch_size
      end_index = min((batch_index+1)*batch_size,data_size)
      return_i = self.intentvalue[start_index:end_index]
      return_nl = self.encoded[start_index:end_index]
      yield (return_i,return_nl)