...
def main():
    ...
    for cur_epoch in range(epoch):
        ...
        for cnt in range(50):
            # get the data
            batch_nl, batch_intent, batch_distance = data.get_train_batch(batch_size, "Guide")
        ...
        for i in range(0, len(batch_nl), batch_size):
            temp_test_output = sess.run(model.intent_output,
                feed_dict={
                    ...
                    model.hist_nl:history_nl[i:i+batch_size],
                    model.hist_nl_len:history_nl_len[i:i+batch_size],
        ...
        test_pred, test_label = calculate_score(test_output, ground_truth, "Guide")
