...
guide_in = tf.placeholder('float', [None, 3, 27])
tourist_in = tf.placeholder('float', [None, 3, 27])
sap_y = tf.placeholder('float',[None, 27])

#define weights
weights = {
    'sap_weight': tf.Variable(tf.random_normal([8*num_hidden_layer, 27]), name='sap_weight')
}
biases = {
...
sap_tourist = sap_birnn(tourist_in,'Tourist')
sap_guide = sap_birnn(guide_in,'Guide')
...
with tf.Session(config=config) as sess:
    ...
    while iterations < epoc:
        ...
        for step,(batch_x,batch_info) in enumerate(Data.get_batch(batch_size)):
            ...
            batch_info = np.transpose(batch_info,(1,0,2))
            tourist_x = batch_x[0:3]
            guide_x = batch_x[3:6]
            tourist_x = np.transpose(tourist_x,(1,0,2))
            guide_x = np.transpose(guide_x,(1,0,2))
            system_action = batch_x[-1]
            _,c,p = sess.run([sap_optimizer,sap_cost,_sap_pred],feed_dict={tourist_in:tourist_x,guide_in:guide_x,sap_y:system_action})
            ...
            if step % 50 == 0 and step != 0:
                ...
                for test_step,(test_batch,test_batch_info) in enumerate(t_data.get_batch(1)):
                    test_batch = np.transpose(test_batch,(1,0,2))
                    test_batch_info = np.transpose(test_batch_info,(1,0,2))
                    test_tourist_x = test_batch[0:3]
                    test_guide_x = test_batch[3:6]
                    test_tourist_x = np.transpose(test_tourist_x,(1,0,2))
                    test_guide_x = np.transpose(test_guide_x,(1,0,2))
                    test_system_action = test_batch[-1]
                    system_action_prediction = sess.run(_sap_pred,feed_dict={tourist_in:test_tourist_x,guide_in:test_guide_x,sap_y:test_system_action})
